...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateImageActionRecords";
        serverCall(url, data, 'post', true, 'updateloader', null, null, function (res) {
            ...
            if (res.length>0) {
                if(res[0].status == true){
                    ...
                    $('.multi_action').hide(); 
                    if(res[0].total_image_records >= 10) {
                        $('.btn-add-new').hide();
                    } else {
                        $('.btn-add-new').show();
                    }
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        serverCall(url, null, 'get', true, null, null, null, function (res) {
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=title]" ).val(res[0].data.title).trigger("change");
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "bannerImageRecords/";
        var method = 'POST';
        var title = $("input[name=title]").val();
        var image_file = $("file[name=image_file]").val();
        var applied_to = $(this).find('input:hidden[name=applied_to]').val();
        ...
        // var formData = new FormData(this);
        formData.append('applied_to', 'BANNER');
    ...
    var dataTable = $('#tblCustomer').DataTable( {
        ...
        "ajax":{
            ...
            complete: function(data) {
                ...
                console.log(json);
                var totalrecord = json.recordsTotal; // or json["Data"]
