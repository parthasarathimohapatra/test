...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateSplashScreenActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        var url = "splash-screen-details/"+ id ;
        serverCall(url, null, 'get', true, null, null, null, function (res) {
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=heading]" ).val(res[0].data.heading).trigger("change");
                    $( "textarea[name=splash_text]" ).val(res[0].data.splash_text).trigger("change"); 
                    ...
                    // });
                     if( res[0].data.image_file != null || res[0].data.image_file !=''){
                        $('#users_av').attr("src", res[0].data.image_file );
                    }else{
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updateSplashScreenRecords/";
        var method = 'POST';
        var heading = $("input[name=heading]").val();
        var splash_text = $('textarea#splash_text').val();
        ...
        // formData.append('splash_text', 'splash_text');
...
function readURL(input) {
    if (input.files && input.files[0]) {
        ...
        if (type_reg.test(type)) {
            ...
            if( size > 2000){
                // $('.add_popupForm').find('.formResponse').addClass('error-msg').html("Please fill up all required fields");
                $("#image-up").after("<span class='error-input img-upload'>Max. image upload size is 2MB.</span>");
            } else{
                $(".img-upload").remove();
