...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateSubscriptionPlanActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        var url = "subscription-plan-details/"+ id ;
        serverCall(url, null, 'get', true, null, null, null, function (res) {
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=plan_name]" ).val(res[0].data.plan_name).trigger("change");
                    $( "input[name=price]" ).val(res[0].data.price).trigger("change");
                    $( "input[name=currency]" ).val(res[0].data.currency).trigger("change");
                   
                    // if(res[0].data.currency!= null){$( "select[name=currency]" ).val(res[0].data.currency).trigger("change");} 
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updateSubscriptionPlanRecords/";
        var method = 'POST';
...
function formValidation( idVal ){
    ...
    $( "." + idVal ).find("input, select").not(":hidden").each(function( value){
        ...
        if(fieldName && (fieldVal == "" || fieldVal == 0) && errorFields[fieldName]){
...
function refreshData(){
    ...
    var dataTable = $('#tblCustomer').DataTable( {
        ...
        "ajax":{
            ...
            url : "{{ settings.REST_URL }}" +"subscription-plan-list-json",
