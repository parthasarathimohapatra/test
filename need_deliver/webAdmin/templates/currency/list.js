...
function conv_rate_save(id) {
    // alert("id = "+id);
    var convrate_fld_name = "conv_rate_"+id;
    var currency_fld_name = "currency_"+id;
    var url =  "updateCurrConversionRecords/";
        var method = 'POST';
        var conv_rate = $("input[name="+convrate_fld_name+"]").val();
        var currency_fld = $("input[name="+currency_fld_name+"]").val();
        // alert("con = "+conv_rate);
        var csrftoken = $("input[name=csrfmiddlewaretoken]").val();

        // var id = $(this).find('input:hidden[name=id]').val();
        $('.formResponse').removeClass('success-msg').removeClass('error-input').html("");
         ...
         serverCall(url, (data), method, true, 'formloader', null, null, function (res) {
            ...
            if (res.length>0) {
                if(res[0].status == false){
                    $('.formResponse').addClass('error-msg').html(res[0].msg);
                }
                else {
                    $('.formResponse').addClass('success-msg').html(res[0].msg);
                }
...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateLanguageActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        serverCall(url, null, 'get', true, null, null, null, function (res) {
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=lang_name]" ).val(res[0].data.lang_name).trigger("change");
                    if( res[0].data.image_file != null || res[0].data.image_file !=''){
                        $('#users_av').attr("src", "{{BASE_URL}}"+res[0].data.image_file );
                    }else{
