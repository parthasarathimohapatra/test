...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateVehicleModelActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        $(".add_popupForm").find('input:hidden[name=id]').val(id);
        var url = "vehiclemodel-details/"+ id ;
        serverCall(url, null, 'get', true, null, null, null, function (res) {
            $(".modal-body").show();
            $(".edit-data-load-content").hide();
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=model_name]" ).val(res[0].data.model_name).trigger("change");
                    if(res[0].data.vehicle_type.id!= null){$( "select[name=vehicle_type]" ).val(res[0].data.vehicle_type.id).trigger("change");} 
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updateVehiclemodelRecords/";
        var method = 'POST';
