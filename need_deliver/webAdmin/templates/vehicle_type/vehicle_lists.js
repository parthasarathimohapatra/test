...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateVehicleActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        $(".add_popupForm").find('input:hidden[name=id]').val(id);
        var url = "vehicletype-details/"+ id ;
        serverCall(url, null, 'get', true, null, null, null, function (res) {
            $(".modal-body").show();
            $(".edit-data-load-content").hide();
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=vehicle_type_name]" ).val(res[0].data.vehicle_type_name).trigger("change");
                    $( "input[name=base_fare]" ).val(res[0].data.base_fare).trigger("change");
                    $( "input[name=khr]" ).val(res[0].data.khr).trigger("change");
                    $( "input[name=person_capcity]" ).val(res[0].data.person_capcity).trigger("change");
                    if( res[0].data.vehicle_logo != null || res[0].data.vehicle_logo !=''){
                        $('#vehicle_logo_show').attr("src", res[0].data.vehicle_logo );
                    }else{
                         $("#add_popupForm").find('#vehicle_logo_show').attr('src', "{% static 'images/avatar.jpg' %}"); 
                    }
                    if( res[0].data.top_view_logo != null || res[0].data.top_view_logo !=''){
                        $('#top_view_show').attr("src", res[0].data.top_view_logo );
                    }else{
                         $("#add_popupForm").find('#top_view_show').attr('src', "{% static 'images/avatar.jpg' %}"); 
                    }
                    $(".add_popupForm").find('.big-img-loader').each(function(){
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updateVehicletypeRecords/";
        var method = 'POST';
        ...
        formData.append('vehicle_logo', $('#vehicle_logo')[0].files[0]);
        formData.append('top_view_logo', $('#top_view_logo')[0].files[0]);
...
function formValidation( idVal ){
    ...
    var flag = false;
    $( "." + idVal ).find("input, select, file").not(":hidden").each(function( value){
...
function clearFormData( formClass ){
    ...
    $( "#vehicle_logo" ).val(null);
    $( "#top_view_logo" ).val(null);
...
function readURL(input, image_show_point) {
    if (input.files && input.files[0]) {
        ...
        if (type_reg.test(type)) {
            ...
            if( size > 7000){
                ...
                $('#'+image_show_point).attr('src', e.target.result);
            }
        };
