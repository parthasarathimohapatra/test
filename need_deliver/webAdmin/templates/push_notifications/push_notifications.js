...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updatePushNotificationActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        $(".add_popupForm").find('input:hidden[name=id]').val(id);
        var url = "push-notification-details/"+ id ;
        serverCall(url, null, 'get', true, null, null, null, function (res) {
            $(".modal-body").show();
            $(".edit-data-load-content").hide();
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=message_title]" ).val(res[0].data.message_title).trigger("change");
                    $( "input[name=message_body]" ).val(res[0].data.message_body).trigger("change");
                    if(res[0].data.role.id!= null){$( "select[name=role]" ).val(res[0].data.role.id).trigger("change");} 
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updatePushNotificationRecords/";
        var method = 'POST';
...
function refreshData(){
    ...
    var dataTable = $('#tblCustomer').dataTable( {
        ...
        "ajax":{
            ...
            url :"push_notification_list_json",
