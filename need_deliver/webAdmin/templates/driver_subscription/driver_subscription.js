...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateDriverSubscriptionActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        if( action == 'view' ){
            ...
            $('#createModal').find('#modal_label').html("Assign Plan to Driver");
        ...
        var url = "driver-subscription-details/"+ id ;
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    ...
                    $( "input[name=driver_name]" ).val(res[0].data.user.first_name+" " +res[0].data.user.last_name).trigger("change");
                    $( "select[name=plan_name]" ).val(res[0].data.subscription_plan.id).trigger("change");
                    // var current_bal = parseFloat(res[0].data.account_balance) + parseFloat(res[0].data.subscription_plan.price);
                    
                    // alert("current_bal = "+current_bal);
                    $( "#prev_account_balance" ).val(res[0].data.account_balance).trigger("change");
                    $( "#prev_account_balance_currency" ).val(res[0].data.account_balance_currency.id).trigger("change");
    ...
    // Start price changing according to plan
    // $( "body" ).on("change", "#plan_name", function(){
    // $( "#plan_name" ).change(function() {
    $("#plan_name").on('change', function() {
        $('#subscription_dtls').hide();
        $('#plan_cal_load').show();
        var plan_id = $( "select[name=plan_name]" ).val();
        var prev_plan_id = ($("#prev_account_balance_currency").val()?$("#prev_account_balance_currency").val():0);
        
        if(plan_id){
            admOptionValue = document.getElementById("custom").value;
            if(admOptionValue == plan_id){
                document.getElementById("custom_price_div").style.display = "block";
            }
            else{
        ...
        var url = "getSubscriptionPlanDetails/"+ plan_id+"/"+prev_plan_id ;
       // alert("url = "+url);
        serverCall(url, null, 'get', true, null, null, null, function (res) {
            $(".modal-body").show();
            $(".edit-data-load-content").hide();
            if (res.length>0) {
                // alert(JSON.stringify(res[0].data));
                if(res[0].status == true){
                    // alert($("#prev_account_balance").val());
                    var prev_bal = ( isNaN(parseFloat($("#prev_account_balance").val()))?0:parseFloat($("#prev_account_balance").val()));
                    var plan_amt = ( isNaN(parseFloat(res[0].data.current_plan.price))?0:parseFloat(res[0].data.current_plan.price));
                    // alert(plan_amt)
                    current_currency = res[0].data.current_plan.ndcurrency.currency;
                    current_currency_id = res[0].data.current_plan.ndcurrency.id;
                    priceConvertedToPrevCurrency = plan_amt;
                    convertedToRequired = 0;
                    // alert("plan_amt = "+plan_amt);
                    // alert("convertedToRequired = "+convertedToRequired);
                    // Start Currency conversion code
                    // alert("res[0].data.current_plan.ndcurrency.conversion_rate "+res[0].data.current_plan.ndcurrency.conversion_rate);
                   
                    if(res[0].data.prev_plan.plan_name!=""){
                        current_conversion_rate = isNaN(parseFloat(res[0].data.prev_plan.ndcurrency.conversion_rate))?0:parseFloat(res[0].data.prev_plan.ndcurrency.conversion_rate);
                        // alert("in if cond current_conversion_rate = "+current_conversion_rate);
                        if(current_conversion_rate != 0) {

                            // alert("plan_amt /current_conversion_rate = "+plan_amt+" /"+current_conversion_rate)
                            priceConvertedToPrevCurrency = plan_amt /current_conversion_rate;
                            // alert("priceConvertedToPrevCurrency = "+priceConvertedToPrevCurrency);
                            would_be_conversion_rate =  isNaN(parseFloat(res[0].data.current_plan.ndcurrency.conversion_rate))?0:parseFloat(res[0].data.current_plan.ndcurrency.conversion_rate);
                            // alert("priceConvertedToPrevCurrency * would_be_conversion_rate = "+priceConvertedToPrevCurrency +"* "+would_be_conversion_rate);
                            convertedToRequired = priceConvertedToPrevCurrency * would_be_conversion_rate;
                            // alert("convertedToRequired = "+convertedToRequired);
                        } 
                        
                    }
                    else {
                        convertedToRequired = plan_amt;
                    }
                    ...
                    // alert("prev_bal + convertedToRequired = "+prev_bal+" + " + convertedToRequired);
                    current_bal = prev_bal + convertedToRequired;
                    details_html = "Plan Price:<span style='color:#cc5200'>"+Number(convertedToRequired).toFixed(2)+" "+current_currency+"</span> <br>Available Balance will be:<span style='color:#006600'>"+Number(current_bal).toFixed(2)+" "+current_currency+"</span>";
                    $( "input[name=account_balance]" ).val(parseFloat(current_bal)).trigger("change");
                    $( "input[name=account_balance_currency]" ).val(current_currency_id).trigger("change");
                    $('#subscription_dtls').show();

                    $("#subscription_dtls").html(details_html);
                    $('#plan_cal_load').hide();
    ...
    // End price changing according to plan
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updateDriverSubscriptionRecords/";
        var method = 'POST';
...
function refreshData(){
    ...
    var dataTable = $('#tblCustomer').dataTable( {
        ...
        "ajax":{
            ...
            url :"driver_subscription_list_json",
...
function clearFormData( formClass ){
    ...
    $( "." + formClass ).find("input,textarea,hidden").not( ":file" ).val('').trigger('change');
...
function addCustomPrice(val)
{
    // alert(val);
    
    var prev_bal = ( isNaN(parseFloat($("#prev_account_balance").val()))?0:parseFloat($("#prev_account_balance").val()));
    var plan_amt = ( isNaN(parseFloat(val))?0:parseFloat(val));
    current_bal = prev_bal + plan_amt;
    details_html = "Plan Price:<span style='color:#cc5200'>"+Number(plan_amt).toFixed(2)+" "+current_currency+"</span> <br>Available Balance will be:<span style='color:#006600'>"+Number(current_bal).toFixed(2)+" "+current_currency+"</span>";
    $( "input[name=account_balance]" ).val(parseFloat(current_bal)).trigger("change");
    // $( "input[name=account_balance_currency]" ).val(current_currency_id).trigger("change");
    $('#subscription_dtls').show();
