...
$(function(){
    ...
    $( "body" ).on("click", ".action-btn", function(){
        ...
        var url = "updateReviewReasonActionRecords";
    ...
    $( "body" ).on("click", ".editView", function(){
        ...
        var url = "review-reason-details/"+ id ;
        serverCall(url, null, 'get', true, null, null, null, function (res) {
           ...
           // return false;
            if (res.length>0) {
                if(res[0].status == true){
                    $( "input[name=review_reason]" ).val(res[0].data.review_reason).trigger("change");
	...
	$('body').on('submit', '.add_popupForm', function (e) { 
        ...
        var url =  "updateReviewReasonRecords/";
        var method = 'POST';
        var review_reason = $("input[name=review_reason]").val();
