...
// Start of vehhicle type respective model
    $('#vehicle_type').on('change',function(){
        var html = '<option value="0"> Loading... </option>';
        $('#vehicle_model').html(html); 
        var vehicleID = $(this).val();
        if(vehicleID !=0 || vehicleID){
            var data = null;
            // var url = "{{ WEBADMIN }}" + "/states/"+ vehicleID;
            var url = "get-vehiclemodel-by-vtype/"+ vehicleID ;
            serverCall(url, null, 'get', true, null, null, null, function (res) {
                if (res.length>0) {
                    // var resp = $.parseJSON(data);alert(1);
                    // $.each(res, function(key, val){
                    //     $.each(val, function(key1, val1){
                            ...
                            if(res[0].status  == true){
                                // data.vehicle_details.vehicle_model.id
                                var html = '<option value="0">-- Vehicle Model --</option>';
                                $.each(res[0].data, function(key2, val2){
                                    selected=""
                                    if($('#existing_vehicle_model').val()==val2['id'])  selected="selected"
                                    html += '<option value="'+ val2['id'] +'" '+selected+'>'+ val2['model_name'] +'</option>';
                                });
                                $('#vehicle_model').html(html); 
                            } else if(res[0].data == false){
                                $('#vehicle_model').html('<option value="0">-- Vehicle Model --</option>');
                            }
                    //     });    
                    // });
                }
...
// End of vehhicle type respective model
            ...
            if (res.length>0) {
                ...
                //     $.each(val, function(key1, val1){
                        if(res[0].status == true){
                            ...
                            if(res[0].data.vehicle_details.vehicle_model.id!=null){$("#existing_vehicle_model").val(res[0].data.vehicle_details.vehicle_model.id).trigger("change");}
                            // if(res[0].data.vehicle_details.vehicle_model!= null){$( "select[name=vehicle_model]" ).val(res[0].data.vehicle_details.vehicle_model.id).trigger("change");}
